export const getTranslations = (lang) => {
  ...
  t.uploadAuditTitle = lang === 'zh' ? '上传校验报告' : 'Upload Validation Report';
  t.uploadRows = lang === 'zh' ? '处理行数' : 'Rows';
  t.uploadEmployees = lang === 'zh' ? '员工档案' : 'Employees';
  t.uploadProjectHours = lang === 'zh' ? '项目工时' : 'Project Hours';
  t.uploadNonProjectHours = lang === 'zh' ? '非项目工时' : 'Non-project Hours';
  t.uploadTotalHours = lang === 'zh' ? '总工时' : 'Total Hours';
  t.uploadBuOverwrites = lang === 'zh' ? 'BU 修正' : 'BU Fixes';
  t.uploadBuDetails = lang === 'zh' ? 'BU 覆盖' : 'BU overrides';
  t.uploadUnmatchedEmployees = lang === 'zh' ? '未匹配员工' : 'Unmatched employees';
  t.uploadUnmatchedProjects = lang === 'zh' ? '未匹配项目' : 'Unmatched projects';
  t.uploadBlankBuProjects = lang === 'zh' ? '空 BU 项目' : 'Blank-BU projects';
  t.uploadLatestMonth = lang === 'zh' ? '最新月份汇总' : 'Latest month';
  t.uploadNone = lang === 'zh' ? '无' : 'None';
  t.uploadRiskStatus = lang === 'zh' ? '风险状态' : 'Risk Status';
  t.uploadRiskClean = lang === 'zh' ? '干净' : 'Clean';
  t.uploadRiskReview = lang === 'zh' ? '需复核' : 'Review';
  t.uploadRiskAttention = lang === 'zh' ? '需关注' : 'Attention';
  t.uploadDuplicateRows = lang === 'zh' ? '疑似重复' : 'Possible Duplicates';
  t.uploadMonthCount = lang === 'zh' ? '月份数' : 'Months';
  t.uploadShowDetails = lang === 'zh' ? '展开明细' : 'Show Details';
  t.uploadHideDetails = lang === 'zh' ? '收起明细' : 'Hide Details';
  t.uploadDownloadAudit = lang === 'zh' ? '下载校验结果' : 'Download Audit';
  t.close = lang === 'zh' ? '关闭' : 'Close';
